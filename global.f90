...
MODULE global
   ...
   TYPE(PARTICLE_DATA_STRUCTURE), DIMENSION(:), ALLOCATABLE :: particles, part_adv
   ...
   TYPE UNSTRUCTURED_GRID_DATA_STRUCTURE
      ...
      INTEGER, DIMENSION(:), ALLOCATABLE        :: CELL_PROC
   ...
   ENUM, BIND(C)
      ENUMERATOR VACUUM, SPECULAR, DIFFUSE, REACT, AXIS, PERIODIC_MASTER, PERIODIC_SLAVE, EMIT
   END ENUM

   ENUM, BIND(C)
      ENUMERATOR DIRICHLET_BC, NEUMANN_BC, ROBIN_BC, PERIODIC_MASTER_BC, PERIODIC_SLAVE_BC, NO_BC
   END ENUM
   ...
   TYPE BOUNDARY_CONDITION_DATA_STRUCTURE
      ...
      REAL(KIND=8) :: WALL_CHARGE = 0.d0
      REAL(KIND=8) :: WALL_CAPACITANCE
...
CONTAINS  ! @@@@@@@@@@@@@@@@@@@@@ SUBROUTINES @@@@@@@@@@@@@@@@@@@@@@@@
   ...
   SUBROUTINE ST_MATRIX_DEALLOCATE(THIS)

      TYPE(ST_MATRIX), INTENT(INOUT) :: THIS

      DEALLOCATE(THIS%RIDX)
      DEALLOCATE(THIS%CIDX)
      DEALLOCATE(THIS%VALUE)
      THIS%NNZ = 0

   END SUBROUTINE ST_MATRIX_DEALLOCATE
   ...
   SUBROUTINE CC_MATRIX_DEALLOCATE(THIS)

      TYPE(CC_MATRIX), INTENT(INOUT) :: THIS
   ...
   END SUBROUTINE CC_MATRIX_DEALLOCATE
