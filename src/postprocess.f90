MODULE postprocess
   ...
   SUBROUTINE TALLY_PARTICLE_TO_BOUNDARY(REFLECTED, PARTICLE, IC, IFACE)
      ...
      MOLMASS = SPECIES(PARTICLE%S_ID)%MOLECULAR_MASS

      IF (DIMS == 2) THEN
         INDEX = U2D_GRID%CELL_EDGES_BOUNDARY_INDEX(IFACE, IC) + (PARTICLE%S_ID-1)*NBOUNDARY
         AREA = U1D_GRID%SEGMENT_AREAS(U2D_GRID%CELL_EDGES_BOUNDARY_INDEX(IFACE, IC))
      ELSE IF (DIMS == 3) THEN
         INDEX = U3D_GRID%CELL_FACES_BOUNDARY_INDEX(IFACE, IC) + (PARTICLE%S_ID-1)*NBOUNDARY
      ...
      IF (REFLECTED) THEN
         ...
         TIMESTEP_PXOUT(INDEX) = TIMESTEP_PXOUT(INDEX) - K * PARTICLE%VX
         TIMESTEP_PYOUT(INDEX) = TIMESTEP_PYOUT(INDEX) - K * PARTICLE%VY
         TIMESTEP_PZOUT(INDEX) = TIMESTEP_PZOUT(INDEX) - K * PARTICLE%VZ
      ...
      ELSE
         ...
         TIMESTEP_PXIN(INDEX) = TIMESTEP_PXIN(INDEX) + K * PARTICLE%VX
         TIMESTEP_PYIN(INDEX) = TIMESTEP_PYIN(INDEX) + K * PARTICLE%VY
         TIMESTEP_PZIN(INDEX) = TIMESTEP_PZIN(INDEX) + K * PARTICLE%VZ
   ...
   SUBROUTINE BOUNDARY_GATHER
      ...
      REAL(KIND=8) :: DBLE_BOUNDARY_AVG_CUMULATED, NUMPART, SAMPLEDOF
      REAL(KIND=8) :: CX, CY, CZ, C2, MASS, VOL, CFNUM


      ! Collect data from all the processes
      IF (PROC_ID .EQ. 0) THEN
         CALL MPI_REDUCE(MPI_IN_PLACE,  TIMESTEP_NIN,   NBOUNDARY*N_SPECIES, MPI_DOUBLE_PRECISION, MPI_SUM, 0, MPI_COMM_WORLD, ierr)
         CALL MPI_REDUCE(MPI_IN_PLACE,  TIMESTEP_NOUT,  NBOUNDARY*N_SPECIES, MPI_DOUBLE_PRECISION, MPI_SUM, 0, MPI_COMM_WORLD, ierr)
         CALL MPI_REDUCE(MPI_IN_PLACE,  TIMESTEP_PXIN,  NBOUNDARY*N_SPECIES, MPI_DOUBLE_PRECISION, MPI_SUM, 0, MPI_COMM_WORLD, ierr)
         CALL MPI_REDUCE(MPI_IN_PLACE,  TIMESTEP_PYIN,  NBOUNDARY*N_SPECIES, MPI_DOUBLE_PRECISION, MPI_SUM, 0, MPI_COMM_WORLD, ierr)
         CALL MPI_REDUCE(MPI_IN_PLACE,  TIMESTEP_PZIN,  NBOUNDARY*N_SPECIES, MPI_DOUBLE_PRECISION, MPI_SUM, 0, MPI_COMM_WORLD, ierr)
         CALL MPI_REDUCE(MPI_IN_PLACE,  TIMESTEP_PXOUT, NBOUNDARY*N_SPECIES, MPI_DOUBLE_PRECISION, MPI_SUM, 0, MPI_COMM_WORLD, ierr)
         CALL MPI_REDUCE(MPI_IN_PLACE,  TIMESTEP_PYOUT, NBOUNDARY*N_SPECIES, MPI_DOUBLE_PRECISION, MPI_SUM, 0, MPI_COMM_WORLD, ierr)
         CALL MPI_REDUCE(MPI_IN_PLACE,  TIMESTEP_PZOUT, NBOUNDARY*N_SPECIES, MPI_DOUBLE_PRECISION, MPI_SUM, 0, MPI_COMM_WORLD, ierr)
         CALL MPI_REDUCE(MPI_IN_PLACE,  TIMESTEP_EIN,   NBOUNDARY*N_SPECIES, MPI_DOUBLE_PRECISION, MPI_SUM, 0, MPI_COMM_WORLD, ierr)
         CALL MPI_REDUCE(MPI_IN_PLACE,  TIMESTEP_EOUT,  NBOUNDARY*N_SPECIES, MPI_DOUBLE_PRECISION, MPI_SUM, 0, MPI_COMM_WORLD, ierr)
      ELSE
         CALL MPI_REDUCE(TIMESTEP_NIN,  TIMESTEP_NIN,   NBOUNDARY*N_SPECIES, MPI_DOUBLE_PRECISION, MPI_SUM, 0, MPI_COMM_WORLD, ierr)
         CALL MPI_REDUCE(TIMESTEP_NOUT, TIMESTEP_NOUT,  NBOUNDARY*N_SPECIES, MPI_DOUBLE_PRECISION, MPI_SUM, 0, MPI_COMM_WORLD, ierr)
         CALL MPI_REDUCE(TIMESTEP_PXIN, TIMESTEP_PXIN,  NBOUNDARY*N_SPECIES, MPI_DOUBLE_PRECISION, MPI_SUM, 0, MPI_COMM_WORLD, ierr)
         CALL MPI_REDUCE(TIMESTEP_PYIN, TIMESTEP_PYIN,  NBOUNDARY*N_SPECIES, MPI_DOUBLE_PRECISION, MPI_SUM, 0, MPI_COMM_WORLD, ierr)
         CALL MPI_REDUCE(TIMESTEP_PZIN, TIMESTEP_PZIN,  NBOUNDARY*N_SPECIES, MPI_DOUBLE_PRECISION, MPI_SUM, 0, MPI_COMM_WORLD, ierr)
         CALL MPI_REDUCE(TIMESTEP_PXOUT,TIMESTEP_PXOUT, NBOUNDARY*N_SPECIES, MPI_DOUBLE_PRECISION, MPI_SUM, 0, MPI_COMM_WORLD, ierr)
         CALL MPI_REDUCE(TIMESTEP_PYOUT,TIMESTEP_PYOUT, NBOUNDARY*N_SPECIES, MPI_DOUBLE_PRECISION, MPI_SUM, 0, MPI_COMM_WORLD, ierr)
         CALL MPI_REDUCE(TIMESTEP_PZOUT,TIMESTEP_PZOUT, NBOUNDARY*N_SPECIES, MPI_DOUBLE_PRECISION, MPI_SUM, 0, MPI_COMM_WORLD, ierr)
         CALL MPI_REDUCE(TIMESTEP_EIN,  TIMESTEP_EIN,   NBOUNDARY*N_SPECIES, MPI_DOUBLE_PRECISION, MPI_SUM, 0, MPI_COMM_WORLD, ierr)
         CALL MPI_REDUCE(TIMESTEP_EOUT, TIMESTEP_EOUT,  NBOUNDARY*N_SPECIES, MPI_DOUBLE_PRECISION, MPI_SUM, 0, MPI_COMM_WORLD, ierr)
      END IF
   ...
   SUBROUTINE BOUNDARY_SAVE
      ...
      IF (PROC_ID .EQ. 0) THEN
         ...
         IF (BOOL_BINARY_OUTPUT) THEN
            ...
            WRITE(54321) 'CELL_DATA '//ITOA(NBOUNDARY)//ACHAR(10)

            WRITE(54321) 'FIELD FieldData '//ITOA( 10*N_SPECIES + 1 )//ACHAR(10)

            ! Write per-cell value
            WRITE(54321) 'PHYSICAL_GROUP '//ITOA(1)//' '//ITOA(NBOUNDARY)//' integer'//ACHAR(10)
            IF (GRID_TYPE == UNSTRUCTURED .AND. DIMS == 2) THEN
            ...
            ! Write per-cell, per-species values
            DO JS = 1, N_SPECIES
               FIRST = 1 + (JS-1)*NBOUNDARY
               LAST  = JS*NBOUNDARY
            
               WRITE(string, *) 'flux_in_', SPECIES(JS)%NAME
               WRITE(54321) string//' '//ITOA(1)//' '//ITOA(NBOUNDARY)//' double'//ACHAR(10)
         ...
         ELSE  ! Write ASCII output.
            ...
            WRITE(54321,'(A,I10)') 'CELL_DATA', NBOUNDARY

            WRITE(54321,'(A,I10)') 'FIELD FieldData', 10*N_SPECIES + 1

            ! Write per-cell value
            WRITE(54321,'(A,I10,I10,A8)') 'PHYSICAL_GROUP', 1, NBOUNDARY, 'integer'
            IF (GRID_TYPE == UNSTRUCTURED .AND. DIMS == 2) THEN
            ...
            DO JS = 1, N_SPECIES
               ...
               WRITE(string, *) 'flux_in_', SPECIES(JS)%NAME
               WRITE(54321,'(A,I10,I10,A7)') string, 1, NBOUNDARY, 'double'
