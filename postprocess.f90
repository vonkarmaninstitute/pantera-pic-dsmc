  ...
  SUBROUTINE GRID_AVG
    ...
    DO JC = 1, NCELLS
      DO JS = 1, NSPECIES
        INDEX = JC+NCELLS*(JS-1)
        IF (TIMESTEP_NP(INDEX) .GT. 0) THEN
          TIMESTEP_TTRX(INDEX) = SPECIES(JS)%MOLMASS / KB * (TIMESTEP_VX2(INDEX) / DBLE(TIMESTEP_NP(INDEX)) &
                                - TIMESTEP_VX(INDEX)*TIMESTEP_VX(INDEX))
          TIMESTEP_TTRY(INDEX) = SPECIES(JS)%MOLMASS / KB * (TIMESTEP_VY2(INDEX) / DBLE(TIMESTEP_NP(INDEX)) &
                                - TIMESTEP_VY(INDEX)*TIMESTEP_VY(INDEX))
          TIMESTEP_TTRZ(INDEX) = SPECIES(JS)%MOLMASS / KB * (TIMESTEP_VZ2(INDEX) / DBLE(TIMESTEP_NP(INDEX)) &
  ...
  SUBROUTINE CHECKS
    ...
    DO JP = 1, NP_PROC
      ...
      ! Momentum
      TOT_MOMENTUM(1) = TOT_MOMENTUM(1) + SPECIES(JS)%MOLMASS*particles(JP)%VX
      TOT_MOMENTUM(2) = TOT_MOMENTUM(2) + SPECIES(JS)%MOLMASS*particles(JP)%VY
      TOT_MOMENTUM(3) = TOT_MOMENTUM(3) + SPECIES(JS)%MOLMASS*particles(JP)%VZ

      ! Kinietic energy
      TOT_KE = TOT_KE + 0.5*SPECIES(JS)%MOLMASS*(particles(JP)%VX**2+particles(JP)%VY**2+particles(JP)%VZ**2)
