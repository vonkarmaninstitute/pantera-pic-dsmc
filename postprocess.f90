MODULE postprocess
   ...
   SUBROUTINE CHECKS
      ...
      REAL(KIND=8)                       :: TOT_KE, TOT_IE, TOT_FE, TOT_EE_FIELD, PHI, CURRENT_TIME
      REAL(KIND=8)                       :: CFNUM
      ...
      IF (PROC_ID .EQ. 0) THEN
         ...
         IF (GRID_TYPE == UNSTRUCTURED) THEN
            TOT_EE_FIELD = 0.d0
            DO JC = 1, NCELLS
               TOT_EE_FIELD = TOT_EE_FIELD + (E_FIELD(1, 1, JC)**2 + E_FIELD(2, 1, JC)**2 + E_FIELD(3, 1, JC)**2)*CELL_VOLUMES(JC)
            END DO
         ...
         SUM(TOT_KE_PART) + TOT_IE + TOT_EE_FIELD !TOT_FE, TOT_EE
         CLOSE(54331)
