...
MODULE velocity_distribution
    ...
    TYPE, EXTENDS(VELOCITY_DISTRIBUTION_STRUCTURE) :: MANUAL_VDF
        CONTAINS
        PROCEDURE :: SAMPLE_VELOCITY => SAMPLE_VELOCITY_MANUAL
        PROCEDURE :: SAMPLE_NORMAL => SAMPLE_NORMAL_MANUAL
        PROCEDURE :: FLUX_DISTR => FLUX_DISTR_MANUAL
        PROCEDURE :: BETA => BETA_MANUAL
    END TYPE MANUAL_VDF

    ! Define some global distributions
    TYPE(MAXWELL_VDF) :: MAXWELL_DISTRIBUTION
    TYPE(KAPPA_VDF)   :: KAPPA_DISTRIBUTION
    ...
    SUBROUTINE SAMPLE_VELOCITY_MANUAL(THIS,UX, UY, UZ, TX, TY, TZ, VX, VY, VZ, M)

        IMPLICIT NONE

        CLASS(MANUAL_VDF), INTENT(IN) :: THIS
        REAL(KIND=8), INTENT(IN)    :: UX, UY, UZ, TX, TY, TZ
        REAL(KIND=8), INTENT(IN)    :: M ! Molecular mass
        REAL(KIND=8), INTENT(INOUT) :: VX, VY, VZ
    ...
    END SUBROUTINE SAMPLE_VELOCITY_MANUAL


    FUNCTION SAMPLE_NORMAL_MANUAL(THIS, SN, TINF, M) RESULT(OUT)
    ...
    END FUNCTION SAMPLE_NORMAL_MANUAL

    FUNCTION FLUX_DISTR_MANUAL(THIS, BETA, S_NORM) RESULT(OUT)
        CLASS(MANUAL_VDF), INTENT(IN) :: THIS
        REAL(KIND=8), INTENT(IN) :: BETA, S_NORM
        REAL(KIND=8) :: OUT
        ...
        OUT = 1.d0/(BETA*2.*SQRT(PI)) * (EXP(-S_NORM**2) &
                                + SQRT(PI)*S_NORM*(1.+ERF1(S_NORM)))
    END FUNCTION FLUX_DISTR_MANUAL

    FUNCTION BETA_MANUAL(THIS, TTRA, M) RESULT(OUT)
        CLASS(MANUAL_VDF), INTENT(IN) :: THIS
