...
MODULE postprocess
   ...
   SUBROUTINE GRID_AVG
      ...
      INTEGER :: LENGTH, COLL_LENGTH
      REAL(KIND=8), DIMENSION(:), ALLOCATABLE :: TIMESTEP_SPECIES_COLLISIONS
      REAL(KIND=8), DIMENSION(:), ALLOCATABLE :: TIMESTEP_SPECIES_COLLISIONS_ENERGY
      REAL(KIND=8), DIMENSION(:), ALLOCATABLE :: TIMESTEP_TOT_SPECIES_COLLISIONS
      REAL(KIND=8), DIMENSION(:), ALLOCATABLE :: TIMESTEP_TOT_SPECIES_COLLISIONS_ENERGY
      ...
      ALLOCATE(TIMESTEP_SPECIES_COLLISIONS(COLL_LENGTH))
      ALLOCATE(TIMESTEP_SPECIES_COLLISIONS_ENERGY(COLL_LENGTH))
      ALLOCATE(TIMESTEP_TOT_SPECIES_COLLISIONS(NCELLS))
      ALLOCATE(TIMESTEP_TOT_SPECIES_COLLISIONS_ENERGY(NCELLS))
      ...
      TIMESTEP_SPECIES_COLLISIONS = 0.0
      TIMESTEP_SPECIES_COLLISIONS_ENERGY = 0.0
      TIMESTEP_TOT_SPECIES_COLLISIONS = 0.0
      TIMESTEP_TOT_SPECIES_COLLISIONS_ENERGY = 0.0
         ...
         DO JS = 1, N_SPECIES
            ...
            IF (TIMESTEP_NP(INDEX) .GT. 0) THEN
               ...
               IF (TIMESTEP_NP(INDEX) .GT. 1) THEN
                  ...
                  ! SAMPLEDOE = NUMPART - 1.5 + 1./(8*(NUMPART-1))
                  SAMPLEDOF = NUMPART
      ...
      !Flatten Collision Matrix
      IF (COLLISION_TYPE_STRING .EQ. "DSMC"        .OR. &
         COLLISION_TYPE_STRING  .EQ. "VAHEDI_DSMC" .OR. &
         ...
         DO JC = 1, NCELLS
            ...
            DO JS = 1, PAIR_POSSIBILITIES
               INDEX = JC+NCELLS*(JS-1)
               TIMESTEP_SPECIES_COLLISIONS(INDEX) = collision(JC)%SPECIES_COLLISIONS(JS)/VOL/DT 
             
               IF (TIMESTEP_SPECIES_COLLISIONS(INDEX) .EQ. 0) THEN
                  TIMESTEP_SPECIES_COLLISIONS_ENERGY(INDEX) = 0.0
               ELSE
                  TIMESTEP_SPECIES_COLLISIONS_ENERGY(INDEX) = collision(JC)%SPECIES_COLLISIONS_ENERGY(JS) / &
                  collision(JC)%SPECIES_COLLISIONS(JS)
               END IF 
            END DO 
           
            TIMESTEP_TOT_SPECIES_COLLISIONS(JC) = collision(JC)%TOT_SPECIES_COLLISIONS/VOL/DT
          
            IF (TIMESTEP_TOT_SPECIES_COLLISIONS(JC) .EQ. 0) THEN
               TIMESTEP_TOT_SPECIES_COLLISIONS_ENERGY(JC) = 0.0
            ELSE
               TIMESTEP_TOT_SPECIES_COLLISIONS_ENERGY(JC) = collision(JC)%TOT_SPECIES_COLLISIONS_ENERGY / &
               collision(JC)%TOT_SPECIES_COLLISIONS
            END IF 
         END DO 
      END IF
      ...
      IF (PROC_ID .EQ. 0) THEN
         ...
         IF (COLLISION_TYPE_STRING .EQ. "DSMC"        .OR. &
            ...
            COLLISION_TYPE_STRING .EQ.  "MCC"        .OR. &
            COLLISION_TYPE_STRING .EQ. "VAHEDI_MCC") THEN
            CALL MPI_REDUCE(MPI_IN_PLACE,  TIMESTEP_SPECIES_COLLISIONS, COLL_LENGTH, MPI_DOUBLE_PRECISION, MPI_SUM, 0,&
                 MPI_COMM_WORLD, ierr)
            CALL MPI_REDUCE(MPI_IN_PLACE,  TIMESTEP_SPECIES_COLLISIONS_ENERGY, COLL_LENGTH, MPI_DOUBLE_PRECISION, MPI_SUM, 0,&
                 MPI_COMM_WORLD, ierr)
            CALL MPI_REDUCE(MPI_IN_PLACE,  TIMESTEP_TOT_SPECIES_COLLISIONS, NCELLS, MPI_DOUBLE_PRECISION, MPI_SUM, 0,&
                 MPI_COMM_WORLD, ierr)
            CALL MPI_REDUCE(MPI_IN_PLACE,  TIMESTEP_TOT_SPECIES_COLLISIONS_ENERGY, NCELLS, MPI_DOUBLE_PRECISION, MPI_SUM, 0,&
                 MPI_COMM_WORLD, ierr)
         END IF
         !TALLY = 0
         !DO INDEX = 1, NCELLS*N_SPECIES
      ...
      ELSE
         ...
         IF (COLLISION_TYPE_STRING .EQ. "DSMC"        .OR. &
            ...
            CALL MPI_REDUCE(TIMESTEP_SPECIES_COLLISIONS,  TIMESTEP_SPECIES_COLLISIONS, COLL_LENGTH, MPI_DOUBLE_PRECISION, &
                 MPI_SUM, 0, MPI_COMM_WORLD, ierr)
            CALL MPI_REDUCE(TIMESTEP_SPECIES_COLLISIONS_ENERGY,  TIMESTEP_SPECIES_COLLISIONS_ENERGY, COLL_LENGTH, &
                 MPI_DOUBLE_PRECISION, MPI_SUM, 0, MPI_COMM_WORLD, ierr)
            CALL MPI_REDUCE(TIMESTEP_TOT_SPECIES_COLLISIONS,  TIMESTEP_TOT_SPECIES_COLLISIONS, NCELLS, MPI_DOUBLE_PRECISION, &
                 MPI_SUM, 0, MPI_COMM_WORLD, ierr)
            CALL MPI_REDUCE(TIMESTEP_TOT_SPECIES_COLLISIONS_ENERGY,  TIMESTEP_TOT_SPECIES_COLLISIONS_ENERGY, NCELLS,& 
                 MPI_DOUBLE_PRECISION,  MPI_SUM, 0, MPI_COMM_WORLD, ierr)
         END IF
      END IF
      ...
      AVG_SPECIES_COLLISIONS_ENERGY = (AVG_SPECIES_COLLISIONS_ENERGY*DBLE_AVG_CUMULATED + TIMESTEP_SPECIES_COLLISIONS_ENERGY)/ & 
                                      (AVG_CUMULATED +1.0) 
      AVG_TOT_SPECIES_COLLISIONS = (AVG_TOT_SPECIES_COLLISIONS*DBLE_AVG_CUMULATED + TIMESTEP_TOT_SPECIES_COLLISIONS)/ &
                                   (AVG_CUMULATED +1.0) 
      AVG_TOT_SPECIES_COLLISIONS_ENERGY = (AVG_TOT_SPECIES_COLLISIONS_ENERGY*DBLE_AVG_CUMULATED + & 
      TIMESTEP_TOT_SPECIES_COLLISIONS_ENERGY) / (AVG_CUMULATED +1.0) 
      ...
      IF (COLLISION_TYPE_STRING .EQ. "DSMC"        .OR. &
          ...
          COLLISION_TYPE_STRING .EQ.  "MCC"        .OR. &
          COLLISION_TYPE_STRING .EQ. "VAHEDI_MCC") THEN
         DEALLOCATE(TIMESTEP_SPECIES_COLLISIONS)
         DEALLOCATE(TIMESTEP_SPECIES_COLLISIONS_ENERGY)
         DEALLOCATE(TIMESTEP_TOT_SPECIES_COLLISIONS)
         DEALLOCATE(TIMESTEP_TOT_SPECIES_COLLISIONS_ENERGY)
      END IF 
   ...
   SUBROUTINE GRID_SAVE
      ...
      INTEGER                            :: I, J, JS, FIRST, LAST, JPROC, MOM, INDEX

      INTEGER, DIMENSION(:), ALLOCATABLE :: CELL_PROC_ID
      CHARACTER*64, DIMENSION(:), ALLOCATABLE :: COLLISION_PAIR_NAMES 
      ...
      IF (PROC_ID .EQ. 0) THEN
         ...
         IF (BOOL_BINARY_OUTPUT) THEN
            ...
            IF (BOOL_DUMP_MOMENTS) THEN
               WRITE(54321) 'FIELD FieldData '//ITOA( (13+33)*N_SPECIES+1+2*PAIR_POSSIBILITIES+2 )//ACHAR(10)
            ELSE
               WRITE(54321) 'FIELD FieldData '//ITOA( 13*N_SPECIES+1+2*PAIR_POSSIBILITIES+2 )//ACHAR(10)
            END IF
            ...
            ! Collision Info
            IF (COLLISION_TYPE_STRING .EQ. "DSMC" .OR. &
                COLLISION_TYPE_STRING .EQ. "VAHEDI_DSMC") THEN
               INDEX = 1
               
               IF (.NOT. ALLOCATED(COLLISION_PAIR_NAMES)) THEN
                  ALLOCATE(COLLISION_PAIR_NAMES(PAIR_POSSIBILITIES))
               END IF 

               DO I = 1, DSMC_COLL_PAIR_LENGTH
                  DO J = I, DSMC_COLL_PAIR_LENGTH

                     COLLISION_PAIR_NAMES(INDEX) = TRIM(MIXTURES(DSMC_COLL_MIX)%COMPONENTS(I)%NAME) // '_' // &
                     TRIM(MIXTURES(DSMC_COLL_MIX)%COMPONENTS(J)%NAME)
                     INDEX = INDEX + 1                  
                  END DO 
               END DO 
               ...
               DO JS = 1, PAIR_POSSIBILITIES
                  ...
                  WRITE(string, *) 'Coll_Rate_', COLLISION_PAIR_NAMES(JS)
                  WRITE(54321) string//' '//ITOA(1)//' '//ITOA(NCELLS)//' double'//ACHAR(10)
                  WRITE(54321) AVG_SPECIES_COLLISIONS(FIRST:LAST), ACHAR(10)
                  
                  WRITE(string, *) 'Coll_Energy_', COLLISION_PAIR_NAMES(JS)
                  WRITE(54321) string//' '//ITOA(1)//' '//ITOA(NCELLS)//' double'//ACHAR(10)
                  WRITE(54321) AVG_SPECIES_COLLISIONS_ENERGY(FIRST:LAST), ACHAR(10)
               END DO 
               
               WRITE(string, *) 'Total_Coll_Rate'
               WRITE(54321) string//' '//ITOA(1)//' '//ITOA(NCELLS)//' double'//ACHAR(10)
               WRITE(54321) AVG_TOT_SPECIES_COLLISIONS(:), ACHAR(10)
               
               WRITE(string, *) 'Total_Coll_Energy'
               WRITE(54321) string//' '//ITOA(1)//' '//ITOA(NCELLS)//' double'//ACHAR(10)
               WRITE(54321) AVG_TOT_SPECIES_COLLISIONS_ENERGY(:), ACHAR(10)
            END IF 
   ...
   SUBROUTINE INIT_POSTPROCESS
      ...
      IF (COLLISION_TYPE_STRING .EQ. "DSMC"        .OR. &
         ...
         IF (.NOT. ALLOCATED(AVG_SPECIES_COLLISIONS)) THEN
            ALLOCATE(AVG_SPECIES_COLLISIONS(COLL_LENGTH))
         END IF

         IF (.NOT. ALLOCATED(AVG_SPECIES_COLLISIONS_ENERGY)) THEN
         ALLOCATE(AVG_SPECIES_COLLISIONS_ENERGY(COLL_LENGTH))
         END IF
        
         IF (.NOT. ALLOCATED(AVG_TOT_SPECIES_COLLISIONS)) THEN
            ALLOCATE(AVG_TOT_SPECIES_COLLISIONS(NCELLS))
         END IF
        
         IF (.NOT. ALLOCATED(AVG_TOT_SPECIES_COLLISIONS_ENERGY)) THEN
            ALLOCATE(AVG_TOT_SPECIES_COLLISIONS_ENERGY(NCELLS))
         END IF

         AVG_SPECIES_COLLISIONS = 0.0
         AVG_SPECIES_COLLISIONS_ENERGY = 0.0
         AVG_TOT_SPECIES_COLLISIONS = 0.0
         AVG_TOT_SPECIES_COLLISIONS_ENERGY = 0.0
      END IF 
   ...
   SUBROUTINE BOUNDARY_GATHER
      ...
      REAL(KIND=8) :: DBLE_BOUNDARY_AVG_CUMULATED
      ...
      ! Collect data from all the processes (PHI does not need reduction)
      IF (PROC_ID .EQ. 0) THEN
         ...
         CALL MPI_REDUCE(MPI_IN_PLACE,  TIMESTEP_PXEM,  NBOUNDCELLS,          MPI_DOUBLE_PRECISION,MPI_SUM,0,MPI_COMM_WORLD,ierr)
         CALL MPI_REDUCE(MPI_IN_PLACE,  TIMESTEP_PYEM,  NBOUNDCELLS,          MPI_DOUBLE_PRECISION,MPI_SUM,0,MPI_COMM_WORLD,ierr)
         CALL MPI_REDUCE(MPI_IN_PLACE,  TIMESTEP_PZEM,  NBOUNDCELLS,          MPI_DOUBLE_PRECISION,MPI_SUM,0,MPI_COMM_WORLD,ierr)
      ...
      ELSE
         ...
         CALL MPI_REDUCE(TIMESTEP_PXEM, TIMESTEP_PXEM,  NBOUNDCELLS,          MPI_DOUBLE_PRECISION,MPI_SUM,0,MPI_COMM_WORLD,ierr)
         CALL MPI_REDUCE(TIMESTEP_PYEM, TIMESTEP_PYEM,  NBOUNDCELLS,          MPI_DOUBLE_PRECISION,MPI_SUM,0,MPI_COMM_WORLD,ierr)
         CALL MPI_REDUCE(TIMESTEP_PZEM, TIMESTEP_PZEM,  NBOUNDCELLS,          MPI_DOUBLE_PRECISION,MPI_SUM,0,MPI_COMM_WORLD,ierr)
