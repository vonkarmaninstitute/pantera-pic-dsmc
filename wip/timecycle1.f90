...
!         CALL DUMP_TRAJECTORY_FILE(tID)
   ...
   SUBROUTINE ADVECT
      ...
      REAL(KIND=8) :: COLLDIST 
      REAL(KIND=8) :: xcoll, ycoll, DTCOLL 
      REAL(KIND=8) :: VNR, VX_POST, VY_POST, D_QDMx_av, D_QDMy_av        
      INTEGER, DIMENSION(4) :: NORMWALLX, NORMWALLY
      !REAL(KIND=8) :: XMIN, YMIN, XMAX, YMAX
      real(kind=8), dimension(:), allocatable :: d_qdmx, d_qdmy, n
      real(kind=8) :: ms, mg = 6.63d-26
      LOGICAL, DIMENSION(:), ALLOCATABLE :: COLL
        ms = 1d+17*mg
     TOL = 1e-15
...
vx_post = particles(ip)%vx - 2 * particles(ip)%vx * normwallx(wallcoll) + walls(wallcoll)%wx
vy_post = particles(ip)%vy - 2 * particles(ip)%vy * normwally(wallcoll) + walls(wallcoll)%wy

        COLL(IP) = .TRUE.
        hascollidedw = .true.
        d_qdmx(ip) = -(vx_post-particles(ip)%vx)*ms+d_qdmx(ip)
        d_qdmy(ip) = -(vy_post-particles(ip)%vy)*(6.63e-26)+d_qdmy(ip)

        n(ip) = n(ip)+1
        
        PARTICLES(IP)%VX = VX_POST
        PARTICLES(IP)%VY = VY_POST
 particles(IP)%DTRIM = particles(IP)%DTRIM - DTCOLL
 call move_particle(ip, dtcoll)
!        PARTICLES(IP)%X = Particles(ip)%xcoll + PARTICLES(IP)%VX * PARTICLES(IP)%DTRIM
!        PARTICLES(IP)%Y = Particles(ip)%ycoll + PARTICLES(IP)%VY * PARTICLES(IP)%DTRIM
...
!        D_QDMY_av = sum(D_QDMY)/n
        ...
        DO j = 1,N_WALLS
        WALLS(j)%WX = WALLS(j)%WX + (sum(d_qdmx))*ms!*sum(n)!/n
        WALLS(j)%WY = WALLS(j)%WY + sum(d_qdmy)*sum(n)!/n
!        end do 
!        end if
...
!        do j = 1,n_walls
        ...
        WALLS(j)%X2 = WALLS(j)%X2 + WALLS(j)%WX * DT
        WALLS(j)%Y2 = WALLS(j)%Y2 + WALLS(j)%WY * DT
        END DO
write(*,*) walls(4)%wx
        deallocate(coll)
               
        do ip = 1,np_PROC
        if (particles(ip)%x .gt. walls(1)%x1 .and. particles(ip)%x .lt. walls(1)%x2) then 
                if (particles(ip)%y .gt. walls(4)%y2 .and. particles(ip)%y .lt. walls(4)%y1) then
!write(7,*) tID, 'compenetrazione'
!       call REMOVE_PARTICLE_ARRAY(IP, particles, NP_PROC)
      ...
      DO WHILE (IP .GE. 1)
         ...
         IF (REMOVE_PART(IP)) THEN
            CALL REMOVE_PARTICLE_ARRAY(IP, particles, NP_PROC)
         ELSE if (particles(ip)%x .gt. xmax .or. particles(ip)%x .lt. xmin &
                        .or. particles(ip)%y .lt. ymin .or. particles(ip)%y .gt. ymax) then
                       call REMOVE_PARTICLE_ARRAY(ip, particles, np_proc) 
                       ELSE 
            CALL CELL_FROM_POSITION(particles(IP)%X, particles(IP)%Y, IC)
            particles(IP)%IC = IC
...
do j = 1,N_walls
        ...
        enddo                
enddo
write(7,*) tID, NP_PROC, boundcoll, wallcoll
