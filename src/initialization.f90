...
MODULE initialization
   ...
   SUBROUTINE READINPUT
      ...
      DO
         ...
         IF (line=='Connected_surfaces:') THEN
            READ(in1,'(A)') BC_DEFINITION
            CALL DEF_CONNECT_COND_SURFACE(BC_DEFINITION)
         END IF
   ...
   SUBROUTINE DEF_BOUNDARY_CONDITION(DEFINITION)
      ...
      ELSE IF (STRARRAY(2) == 'thin_dielectric_layer') THEN
         GRID_BC(IPG)%FIELD_BC = THIN_DIELECTRIC_LAYER_BC
         READ(STRARRAY(3), '(ES14.0)') GRID_BC(IPG)%LAYER_THICKNESS
         READ(STRARRAY(4), '(ES14.0)') GRID_BC(IPG)%EPS_REL
   ...
   SUBROUTINE DEF_CONNECT_COND_SURFACE(DEFINITION)
      ...
      IF (ALLOCATED(CONNECTED_COND_SURFACES)) THEN
         ALLOCATE(TEMP_CONNECTED_COND_SURFACES(N_CONNECTED_COND_SURFACES+1)) ! Append the mixture to the list
      ...
      CONNECTED_COND_SURFACES(N_CONNECTED_COND_SURFACES)%N_GROUPS = N_STR
      ALLOCATE(CONNECTED_COND_SURFACES(N_CONNECTED_COND_SURFACES)%GROUP_NAMES(N_STR))
      ALLOCATE(CONNECTED_COND_SURFACES(N_CONNECTED_COND_SURFACES)%GROUP_ID(N_STR))

      DO I = 1, N_STR
         
         IPG = -1
         DO IG = 1, N_GRID_BC
            IF (GRID_BC(IG)%PHYSICAL_GROUP_NAME == STRARRAY(I)) IPG = IG
         END DO
         ...
         ELSE
            CONNECTED_COND_SURFACES(N_CONNECTED_COND_SURFACES)%GROUP_ID(I) = IPG
         END IF
      END DO

   END SUBROUTINE DEF_CONNECT_COND_SURFACE
